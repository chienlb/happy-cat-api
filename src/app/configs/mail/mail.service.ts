...
export class MailService {
    ...
    private compileTemplate(templateName: string, context: any): string {
        ...
        if (!templatePath) {
            this.logger.error(`Template not found. Checked paths: ${possiblePaths.join(', ')}`);
            throw new Error(`Template not found: ${templateName}.hbs or ${templateName}.mjml`);
        }
        ...
        this.logger.debug(`Template context username: ${context?.username || 'NOT FOUND'}`);
        const html = compile(context);
        // Check if username was replaced
        if (html.includes('{{username}}') || html.includes('{username}')) {
            this.logger.warn(`Username placeholder not replaced in template. Context: ${JSON.stringify(context)}`);
        }
        return html;
    }
