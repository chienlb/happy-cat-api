...
export class UserBadgesController {
  ...
  @ApiBody({
    type: CreateUserBadgeDto,
    description: 'The user badge to create',
    examples: {
      example1: {
        value: {
          userId: '1234567890',
          badgeId: '1234567890',
          awardedAt: new Date(),
          reason: 'Reason for awarding the badge',
          awardedBy: '1234567890',
          isRevoked: false,
          revokedAt: null,
          note: 'Note for the badge',
        },
      },
    },
  })
  @ApiResponse({
    status: 201,
    description: 'The user badge has been successfully created.',
    type: CreateUserBadgeDto,
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiParam({
    name: 'id',
    type: String,
    description: 'The ID of the user badge',
  })
  @ApiResponse({
    status: 200,
    description: 'The user badge has been successfully retrieved.',
    type: CreateUserBadgeDto,
  })
  @ApiResponse({
    status: 400,
  ...
  async findUserBadgeById(
    @Param('id') id: string,
  ): Promise<CreateUserBadgeDto> {
  ...
  async findAllUserBadges(@Query() paginationDto: PaginationDto) {
    const result =
      await this.userBadgesService.findAllUserBadges(paginationDto);
    return result;
  }
  ...
  @ApiBody({
    type: UpdateUserBadgeDto,
    description: 'The user badge to update',
    examples: {
      example1: {
        value: {
          userId: '1234567890',
  ...
  @ApiResponse({
    status: 200,
    description: 'The user badge has been successfully updated.',
    type: CreateUserBadgeDto,
  })
  @ApiResponse({
    status: 400,
  ...
  async updateUserBadgeById(
    @Param('id') id: string,
    @Body() updateUserBadgeDto: UpdateUserBadgeDto,
  ): Promise<CreateUserBadgeDto> {
    const userBadge = await this.userBadgesService.updateUserBadge(
      id,
      updateUserBadgeDto,
    );
    return {
  ...
  @ApiResponse({
    status: 200,
    description: 'The user badge has been successfully deleted.',
    type: CreateUserBadgeDto,
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiResponse({
    status: 200,
    description: 'The user badge has been successfully revoked.',
    type: CreateUserBadgeDto,
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiResponse({
    status: 200,
    description: 'The user badge has been successfully restored.',
    type: CreateUserBadgeDto,
  })
  @ApiResponse({
    status: 400,
  ...
  async findUserBadgesByUserId(
    ...
    const result = await this.userBadgesService.findUserBadgesByUserId(
      userId,
      paginationDto,
    );
    return result;
